define(function (require, exports, module) {
    ...
    var urls = {
        ...
        //getAllDeptOrgId: CONTEXT_PATH + '/../ydsp/thirdInterface/getAllDeptOrgId',
        //TODO
        ...
        updateDeptByUserId: CONTEXT_PATH + '/thirdInterface/updateDeptByUserId.do',
    ...
    var setUploader = function (button, url, filefilter) {
        ...
        uploader.bind('BeforeUpload', function (_uploader, files) {
            loader.show()
        })
    ...
    var itemRender = template($('#tmpl-employeeRow').html()),
        departModalRender = template($('#tmpl-departEditModal').html()),
        departMoveRender = template($('#tmpl-departMoveModal').html()),
        empModalRender = template($('#tmpl-employeeEditModal').html());
    ...
    var DeptModal = Backbone.View.extend({
        ...
        doSave: function () {
            var _this = this;
            if (!this.$('form[role=edit-depart]').valid()) { return; }
    ...
    var DeptTree = Backbone.View.extend({
            ...
            var setting = {
                ...
                async: {
                    url: function (treeId, treeNode) {
                        ...
                        //else {
                        //    return urls.getAllDeptOrgId + '/' + COMPANY_ID + '/' + COMPANY_ID +
        ...
        dataParser: function (model) {
            var tempMap = {}, data = [];
            model.sort(function (a,b) { return a.showindex - b.showindex; });
        ...
        cacheEls: function () {
            ...
            $(document.body).on('click', function (e) {
                var el = $(e.target);
                if ((!el.is('.right-menu')) && (el.parents('.right-menu').length == 0)) {
                    _this.$rMenu.hide();
        ...
        doAdd: function (isRoot) {
            ...
            this.subview.editModal.show({
                ...
                orgName: '',
                orgId: '',
                previousId: isRoot === true ? '' : (cNode ? cNode.orgId : ''),
                showindex: 0
            });
            this.$rMenu.hide();
        },
        ...
        doSave: function (result) {
            ...
            if (result.success) {
                ...
                //} else {
                    //this.tree.addNodes(cNode, { name: result.model.orgName });
    ...
    var DeptMoveModal = Backbone.View.extend({
        ...
        doSave: function () {
            ...
            data = _.pick(data, 'userId', 'userName', 'post', 'workNumber', 'showindex', 'orgId');
    ...
    var EmployeeModal = Backbone.View.extend({
        ...
        doSave: function () {
            var _this = this;
            if (!this.$('form[role=edit-employee]').valid()) { return; }

            var modelJSON = this.$('form[role=edit-employee]').serializeObject();
            modelJSON.selfCompanyId = COMPANY_ID;
            modelJSON.companyId = COMPANY_ID;
    ...
    var EmployeeCollection = Backbone.Collection.extend({
        ...
        parse: function (resp) {
            ...
            if (_.isObject(resp) && _.isArray(resp.model)) {
                resp.model.sort(function (a,b) { return a.showindex - b.showindex; });
                data = resp.model;
            }
            return data;
        },
    ...
    var run = function () {
        ...
        Backbone.listenTo(empEditModal, 'modify', function (result) {
            if (result.success) {
                ...
                searchQuery.set({ deptId: dept.orgId });
            }
        });
        ...
        Backbone.listenTo(Backbone, 'selectDepart', function (dept) {
            searchQuery.set(_.extend({}, defaultQuery, { deptId: dept.orgId }));
        });
