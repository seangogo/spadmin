define(function (require, exports, module) {
    ...
    var urls = {
        ...
        get: CONTEXT_PATH + '/microApp/customForm/getApprovalById.do',
        manager: CONTEXT_PATH + '/microApp/customForm/manager.do',
        mostType: CONTEXT_PATH + '/microApp/customForm/findMostType.do'
    };
    ...
    var WidgetView = Backbone.View.extend({
        ...
        toDataJSON: function (sequence) {
            if (!this.setting.valid()) {
                ...
                var selectArr = typeof this.data.selects === 'string' ? [this.data.selects] : this.data.selects;
                var model = _.extend({}, emptySettingJson, {
                    ...
                    value: selectArr.length > 0 ? { options: selectArr } : '',
                    jsonData: selectArr.length > 0 ? { options: selectArr } : '',
                });
                return model;
            }
        }
    ...
    var parseJsonToSetting = function (jsonData) {
        ...
        if (typeof jsonData.jsonData === 'string' && jsonData.jsonData.length > 0) {
            jsonDataSelects = JSON.parse(jsonData.jsonData).options;
        }
    ...
    var getMostTypeKey = function (mostTypeKey, callback) {
        $.ajax({
            url: urls.mostType,
            type: 'get',
            dataType: 'json',
    ...
    var run = function () {
        ...
        getMostTypeKey(mostTypeKey, function () {
            if (id && id.length > 0) {
                $.ajax({
                    url: urls.get,
                    type: 'get',
